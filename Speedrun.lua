...
function Speedrun.GetSavedTimer(raidID, step)
	if tonumber(Speedrun.GetCustomTimerStep(raidID, step)) then
			return tonumber(Speedrun.GetCustomTimerStep(raidID, step))
	else
			if tonumber(Speedrun.GetSavedTimerStep(raidID, step)) then
					return tonumber(Speedrun.GetSavedTimerStep(raidID, step))
			end
	end
end
...
function Speedrun.FormatTimerForChatUpdate(timer)
	...
	local ms = string.format("%0d", timer % 1000)
	local chatString = h .. m .. ":" .. s .. " (+" .. ms .. "ms)"
	return chatString
end
...
function Speedrun.GetScore(timer, vitality, raidID)
	...
	-- RG
	elseif raidID == 1263 then return (220000 + (1000 * vitality)) * (1 + (2700 - timer) / 10000)
...
function Speedrun.UpdateWaypointNew(raidDuration)
	...
	if raid then
		if not Speedrun.Data.stepList[raid.id][waypoint] then return end
		...
		Speedrun:dbg(1, '[|ce6b800<<1>>|r] |c00ff00Step <<2>>|r at |cffffff<<3>>|r.', GetUnitZone('player'), waypoint, Speedrun.FormatTimerForChatUpdate(GetRaidDuration()))
	end
end
	...
	for k, v in pairs(Speedrun.scores) do
		if Speedrun.scores[k] == scoreUpdateReason or Speedrun.scores[k].id == scoreUpdateReason then
			...
			if scoreUpdateReason ~= 9 then
					Speedrun:dbg(3, 'Score Updated at |cffffff<<4>>|r.\nType: |cffffff<<1>>|r. Amount: |cffffff<<2>>|r. Total: |cffffff<<3>>|r.', Speedrun.scores[k].name, scoreAmount, totalScore, sT)
			end
		end
	end
...
function Speedrun.SideBoss()
	...
	if currentTargetHP <= 0 then
		Speedrun:dbg(1, "|cdf4242<<1>>|r killed at |cffff00<<2>>|r", GetUnitName("boss1"), Speedrun.FormatRaidTimer(GetRaidDuration(), true))
		Speedrun.lastBossName = Speedrun.currentBossName
		sV.lastBossName = Speedrun.lastBossName
		EM:UnregisterForUpdate(Speedrun.name .. "SideBoss")
	end
...
function Speedrun.MainCloudrest()
		if isZmaja then
			...
			if Speedrun.Step == 2 then
				if (percentageHP <= 0.75) then
					Speedrun.UpdateWaypointNew(GetRaidDuration())
					return
				end

			elseif Speedrun.Step == 3 then
				if percentageHP <= 0.5 then
					Speedrun.UpdateWaypointNew(GetRaidDuration())
					return
				end

			elseif Speedrun.Step == 4 then
				if percentageHP <= 0.25 then
					Speedrun.UpdateWaypointNew(GetRaidDuration())
					EM:UnregisterForEvent(Speedrun.name .. "BossChangeCR", EVENT_BOSSES_CHANGED)
		...
		-- 				if (maxTargetHP < 64000000) or IsUnitAttackable("boss" .. i) then return end
		--
		...
		-- 						if Speedrun.Step == 1 then
		-- 								if (currentTargetHP < effmaxTargetHP) and (maxTargetHP > 64000000) then
		-- 										Speedrun.UpdateWaypointNew(GetRaidDuration()) end
		-- 								end
		-- 						end
		-- 						if ( percentageHP <= 0.75 and Speedrun.Step == 2 ) then Speedrun.UpdateWaypointNew(GetRaidDuration()) end
		-- 						if ( percentageHP <= 0.5  and Speedrun.Step == 3 ) then Speedrun.UpdateWaypointNew(GetRaidDuration()) end
		-- 						if ( percentageHP <= 0.25 and Speedrun.Step == 4 ) then	Speedrun.UpdateWaypointNew(GetRaidDuration())
		-- 								EM:UnregisterForEvent(Speedrun.name .. "BossChangeCR", EVENT_BOSSES_CHANGED)
		-- 								EM:RegisterForEvent(Speedrun.name .. "BossChangeCR", EVENT_BOSSES_CHANGED, Speedrun.ZmajaShade)
		-- 						end
		-- 						-- if ((Speedrun.currentBossName ~= string.lower(GetUnitName("boss" .. i))) and Speedrun.Step == 5) then
		-- 						-- 		--ZMaja Shadow
		--             --     Speedrun.UpdateWaypointNew(GetRaidDuration())
		--             -- end
		-- 				-- else
		-- 				-- 	  if (currentTargetHP == maxTargetHP and Speedrun.Step <= 6) then
		--         --         Speedrun.currentRaidTimer = {}
		-- 				-- 				sV.currentRaidTimer	 = Speedrun.currentRaidTimer
		-- 				-- 				Speedrun.Step = 1
		-- 				-- 				sV.Step = Speedrun.Step
...
function Speedrun.BossFightBegin()
	for i = 1, MAX_BOSSES do
		local currentTargetHP, maxTargetHP, effmaxTargetHP = GetUnitPower("boss" .. i, POWERTYPE_HEALTH)
		if IsUnitInCombat("player") and (currentTargetHP < maxTargetHP) then
...
local function BossMainZoneCheck(zone)
	if zone == 638 or zone == 639 or zone == 725 or zone == 975 or zone == 1121 or zone == 1196 or zone == 1263 then
		return true
	end
	return false
end
...
Speedrun.OnTrialComplete = function(eventCode, trialName, score, totalTime)
	...
	Speedrun:dbg(1, "|ce6b800<<1>>|r |c00ff00Complete|r!\n[Time: |cffffff<<2>>|r]  [Score: |cffffff<<3>>|r] <<4>>", GetUnitZone('player'), Speedrun.FormatTimerForChatUpdate(totalTime), scoreString, Speedrun.FormatVitality(true, GetRaidReviveCountersRemaining(), GetCurrentRaidStartingReviveCounters()))
end
...
function Speedrun.CheckTrial()
	...
	if NewTrialCheck() then
		Speedrun:dbg(2, "New Trial.")
		shouldReset = true
	else
		if IsActiveTrialOldTrial() then
			Speedrun:dbg(2, "Same Trial.")
		else
			Speedrun:dbg(2, "Trial active (not same).")
			shouldReset = true
		end
	end
